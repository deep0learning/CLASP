...
def get_class_name_from_label(label):
    if label == 'person' or label == 'perron':
        name = 'person'
    elif label == 'bin':
        name = 'bin'
    return name
...
def main(_):
  ...
  train_output_path = os.path.join(FLAGS.output_dir, 'alert_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'alert_val.record')
